...
namespace VideoPlayDemo
{
    class Program : NUIApplication
    {
        ...
        public void LongPanGestureTest()
        {
            ...
            var testGesture = new PanGestureDetector();
            testGesture.Attach(yellowView);
            testGesture.Detected += (s, e) =>
            {
                log.AddLog($" -> testGesture OnPan {e.PanGesture.State} \n");
            };


            bool isDetected = false;
            tempPanGestureDetector = new PanGestureDetector();
            tempPanGestureDetector.Attach(orangeView);
            Color tempColor = yellowView.BackgroundColor * 0.7f;;
            tempPanGestureDetector.Detected += (s, e) =>
            {
                if(e.View == orangeView)
                {
                    log.AddLog($" -> tempPanGestureDetector orangeView OnPan {e.PanGesture.State} {isDetected}\n");
                }
                else if (e.View == yellowView)
                {
                    log.AddLog($" -> tempPanGestureDetector yellowView OnPan {e.PanGesture.State} {isDetected}\n");
                }
