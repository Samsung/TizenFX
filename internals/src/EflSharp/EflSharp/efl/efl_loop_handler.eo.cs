...
private static object BufferEvtKey = new object();
    ...
    private void on_BufferEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
        EventArgs args = EventArgs.Empty;
        try {
            On_BufferEvt(args);
...
private static object PrepareEvtKey = new object();
    ...
    public event EventHandler PrepareEvt
    {
        add {
            lock (eventLock) {
                ...
                if (AddNativeEventHandler(efl.Libs.Ecore, key, this.evt_PrepareEvt_delegate)) {
        ...
        remove {
            lock (eventLock) {
                string key = "_EFL_LOOP_HANDLER_EVENT_PREPARE";
                if (RemoveNativeEventHandler(key, this.evt_PrepareEvt_delegate)) { 
    ...
    private void on_PrepareEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
        EventArgs args = EventArgs.Empty;
        try {
            On_PrepareEvt(args);
