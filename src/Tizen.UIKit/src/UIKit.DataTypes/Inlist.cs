...
public class Inlist<T> : IEnumerable<T>, IDisposable
{
    ...
    {
        return new UIKit.DataTypes.Iterator<T>(eina_inlist_iterator_wrapper_new_custom_export_mono(Handle), true);
    }
    ...
    {
        return new UIKit.DataTypes.AccessorInList<T>(eina_inlist_accessor_new(Handle), Ownership.Managed);
    }
}
