...
namespace Tizen.NUI.Accessibility
{
    ...
    public static class Accessibility
    {
        ...
        {
            var positionVal = new Int32Pair(position.X, position.Y);
            var positionPtr = Int32Pair.getCPtr(positionVal);

            var sizeVal = new Int32Pair(size.Width, size.Height);
            var sizePtr = Int32Pair.getCPtr(sizeVal);


            Interop.ControlDevel.SetCustomHighlightOverlay(View.getCPtr(view), positionPtr, sizePtr);
            if (NDalicPINVOKE.SWIGPendingException.Pending) throw NDalicPINVOKE.SWIGPendingException.Retrieve();
        }

        /// <summary>
        /// Function to reset the custom highlight overlay
        /// This functionality is only applicable when the CustomHighlight Overlay is a child of the scene-view.
        /// </summary>
        // This will be public opened after ACR done. (Before ACR, need to be hidden as Inhouse API)
        [EditorBrowsable(EditorBrowsableState.Never)]
