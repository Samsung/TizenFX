...
TARGET_ASSEMBLY_PATH=$ROOTDIR/test/Tizen.NUI.Tests/Tizen.NUI.Devel.Tests/bin/Debug/netcoreapp3.1
...
prepare_replace() {
  ...
  rm -rf $TARGET_COV_PATH
  mkdir -p $TARGET_COV_PATH
  cp $MINICOVER_PATH/MiniCover.HitServices.dll $TARGET_ASSEMBLY_PATH
}

replace() {
  PROJECT_DIR=$(check_module $1)
  #sdb push $PROJECT_DIR/bin/$BUILD_CONF/*/$1.dll $TARGET_ASSEMBLY_PATH
  cp $PROJECT_DIR/bin/$BUILD_CONF/*/$1.dll $TARGET_ASSEMBLY_PATH
