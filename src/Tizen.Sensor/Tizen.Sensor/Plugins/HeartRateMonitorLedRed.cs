...
namespace Tizen.Sensor
{
    ...
    public sealed class HeartRateMonitorLedRed : Sensor
    {
        ...
        public static bool IsSupported
        {
            get
            {
                Log.Info(Globals.LogTag, "Checking if the HeartRateMonitor Led Red is supported");
                return CheckIfSupported(SensorType.HeartRateMonitorLedRed, HrmKey);
            }
        }
        ...
        public static int Count
        {
            get
            {
                Log.Info(Globals.LogTag, "Getting the count of Led Red heart rate monitors");
                return GetCount();
            }
        }
        ...
        /// <param name='index'>
        /// Index. Default value for this is 0. Index refers to a particular Led Red heart rate monitor in case of multiple sensors.
        /// </param>
        public HeartRateMonitorLedRed(uint index = 0) : base(index)
        {
            Log.Info(Globals.LogTag, "Creating HeartRateMonitorLedRed object");
        }

        internal override SensorType GetSensorType()
        {
            return SensorType.HeartRateMonitorLedRed;
        }
        ...
        private static int GetCount()
        {
            IntPtr list;
            int count;
            int error = Interop.SensorManager.GetSensorList(SensorType.HeartRateMonitorLedRed, out list, out count);
            if (error != (int)SensorError.None)
            {
                Log.Error(Globals.LogTag, "Error getting sensor list for heart rate monitor Led Red");
                count = 0;
            }
            else
        ...
        internal override void EventListenStart()
        {
            _callback = (IntPtr sensorHandle, IntPtr eventPtr, IntPtr data) => {
                ...
                HrmRedLightValue = (int)sensorData.values[0];

                DataUpdated?.Invoke(this, new HeartRateMonitorLedRedDataUpdatedEventArgs((int)sensorData.values[0]));
            };

            int error = Interop.SensorListener.SetEventCallback(ListenerHandle, Interval, _callback, IntPtr.Zero);
            if (error != (int)SensorError.None)
            {
                Log.Error(Globals.LogTag, "Error setting event callback for Led Red heart rate monitor");
        ...
        internal override void EventListenStop()
        {
            ...
            if (error != (int)SensorError.None)
            {
                Log.Error(Globals.LogTag, "Error unsetting event callback for Led Red heart rate monitor");
