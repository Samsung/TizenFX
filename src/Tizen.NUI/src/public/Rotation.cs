...
namespace Tizen.NUI
{
    ...
    public class Rotation : Disposable
    {
        ...
        {
            Vector3 currentAxis = new Vector3();
            Radian currentAngle = new Radian();

            bool ret = Interop.Rotation.Rotation_GetAxisAngle(swigCPtr, Vector3.getCPtr(currentAxis), Radian.getCPtr(currentAngle));
            if (NDalicPINVOKE.SWIGPendingException.Pending) throw NDalicPINVOKE.SWIGPendingException.Retrieve();
