...
/// <param name="style">The widget style to use. See <see cref="Efl.Ui.Widget.SetStyle" /></param>
    ...
    public event EventHandler AnimateBeginEvent
    {
        add
        {
    ...
    /// <summary>Method to raise event AnimateBeginEvent.</summary>
    /// <param name="e">Event to raise.</param>
    public void OnAnimateBeginEvent(EventArgs e)
    {
    ...
    public event EventHandler AnimateDoneEvent
    {
        add
        {
            lock (eflBindingEventLock)
    ...
    /// <summary>Method to raise event AnimateDoneEvent.</summary>
    /// <param name="e">Event to raise.</param>
    public void OnAnimateDoneEvent(EventArgs e)
    {
    ...
    /// <summary>Interactive flip mode.
    /// This selects if the flip should be interactive (allow user to click and drag a side of the flip to reveal the back page and cause it to flip). By default a flip is not interactive. You may also need to set which sides of the flip are &quot;active&quot; for flipping and how much space they use (a minimum of a finger size) with <see cref="Efl.Ui.Flip.SetInteractionDirectionEnabled"/> and <see cref="Efl.Ui.Flip.SetInteractionDirectionHitsize"/>.
    /// 
    /// The four available modes of interaction are <see cref="Efl.Ui.FlipInteraction.None"/>, <see cref="Efl.Ui.FlipInteraction.Rotate"/>, <see cref="Efl.Ui.FlipInteraction.Cube"/> and <see cref="Efl.Ui.FlipInteraction.Page"/>.
    /// 
    /// Note: <see cref="Efl.Ui.FlipInteraction.Rotate"/> cannot trigger <see cref="Efl.Ui.FlipMode.RotateXzCenterAxis"/> or <see cref="Efl.Ui.FlipMode.RotateYzCenterAxis"/> animations. These are only available through the <see cref="Efl.Ui.Flip.Go"/> and <see cref="Efl.Ui.Flip.GoTo"/> methods.</summary>
    /// <returns>The interactive flip mode to use.</returns>
    public virtual Efl.Ui.FlipInteraction GetInteraction() {
    ...
    public virtual void SetInteraction(Efl.Ui.FlipInteraction mode) {
    ...
    public virtual bool GetFrontVisible() {
    ...
    /// Set the amount of the flip that is sensitive to interactive flip, with 0 representing no area in the flip and 1 representing the entire flip. There is however a consideration to be made in that the area will never be smaller than the finger size set (as set in your Elementary configuration), and dragging must always start from the opposite half of the flip (e.g. right half of the flip when dragging to the left).
    /// 
    ...
    public virtual void SetInteractionDirectionHitsize(Efl.Ui.LayoutOrientation dir, double hitsize) {
    ...
    public virtual double GetInteractionDirectionHitsize(Efl.Ui.LayoutOrientation dir) {
    ...
    public virtual void SetInteractionDirectionEnabled(Efl.Ui.LayoutOrientation dir, bool enabled) {
    ...
    public virtual bool GetInteractionDirectionEnabled(Efl.Ui.LayoutOrientation dir) {
    ...
    /// 
    /// Compare with <see cref="Efl.Ui.Flip.GoTo"/> which only switches sides if necessary.</summary>
    /// <param name="mode">The mode type.</param>
    public virtual void Go(Efl.Ui.FlipMode mode) {
    ...
    /// <summary>Runs the flip animation to front or back as selected with <c>front</c>. If the flip is already showing this side, nothing is done (Compare with <see cref="Efl.Ui.Flip.Go"/> which always switches sides).
    /// Flips the front and back contents using the <c>mode</c> animation. This effectively hides the currently visible content and shows he hidden one.</summary>
    /// <param name="front">If <c>true</c>, makes front visible, otherwise makes back.</param>
    /// <param name="mode">The mode type.</param>
    public virtual void GoTo(bool front, Efl.Ui.FlipMode mode) {
...
/// <summary>Efl UI flip mode used by <see cref="Efl.Ui.Flip.Go"/> and <see cref="Efl.Ui.Flip.GoTo"/>.</summary>
[Efl.Eo.BindingEntity]
public enum FlipMode
{
/// <summary>Rotate Y center axis flip mode: Rotates the currently visible content around a vertical axis in the middle of its width, the other content is shown as the other side of the flip.</summary>
RotateYCenterAxis = 0,
/// <summary>Rotate X center axis flip mode: Rotates the currently visible content around a horizontal axis in the middle of its height, the other content is shown as the other side of the flip.</summary>
RotateXCenterAxis = 1,
/// <summary>Rotate XZ center axis flip mode: Rotates the currently visible content around a diagonal axis in the middle of its width, the other content is shown as the other side of the flip.</summary>
RotateXzCenterAxis = 2,
/// <summary>Rotate YZ center axis flip mode: Rotates the currently visible content around a diagonal axis in the middle of its height, the other content is shown as the other side of the flip.</summary>
RotateYzCenterAxis = 3,
/// <summary>Cube left flip mode: Rotates the currently visible content to the left as if the flip was a cube, the other content is shown as the right face of the cube.</summary>
CubeLeft = 4,
/// <summary>Cube right flip mode: Rotates the currently visible content to the right as if the flip was a cube, the other content is shown as the left face of the cube.</summary>
CubeRight = 5,
/// <summary>Cube up flip mode: Rotates the currently visible content up as if the flip was a cube, the other content is shown as the bottom face of the cube.</summary>
CubeUp = 6,
/// <summary>Cube down flip mode: Rotates the currently visible content down as if the flip was a cube, the other content is shown as the upper face of the cube.</summary>
CubeDown = 7,
/// <summary>Page left flip mode: Moves the currently visible content to the left as if the flip was a book, the other content is shown as the page below that.</summary>
PageLeft = 8,
/// <summary>Page right flip mode: Moves the currently visible content to the right as if the flip was a book, the other content is shown as the page below it.</summary>
PageRight = 9,
/// <summary>Page up flip mode: Moves the currently visible content up as if the flip was a book, the other content is shown as the page below it.</summary>
PageUp = 10,
/// <summary>Page down flip mode: Moves the currently visible content down as if the flip was a book, the other content is shown as the page below that.</summary>
PageDown = 11,
/// <summary>Cross fade flip mode: Fades out the currently visible content, while fading in the invisible content.</summary>
CrossFade = 12,
}
}
}
...
/// <summary>Efl UI flip interaction mode used by <see cref="Efl.Ui.Flip.Interaction"/>.</summary>
[Efl.Eo.BindingEntity]
