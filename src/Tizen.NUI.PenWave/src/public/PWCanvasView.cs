...
namespace Tizen.NUI.PenWave
{
    public class PWCanvasView : DirectRenderingGLView
    {
        ...
        {
            ID = Interop.ExtRenderer.CreateCanvas(-1, -1);
            InitialCanvasView();
        }
        ...
        {
            ID = Interop.ExtRenderer.CreateCanvasWithBackgroundImage(backgroundPath);
            InitialCanvasView();
        }

        private void InitialCanvasView()
        {
            Interop.ExtRenderer.SetCurrentCanvas(ID);
            ...
            this.RegisterGLCallbacks(Interop.ExtRenderer.InitializeGL, OnRender, Interop.ExtRenderer.TerminateGL);
            // RegisterGLCallbacks(Interop.ExtRenderer.InitializeGL, Interop.ExtRenderer.RenderFrameGL, Interop.ExtRenderer.TerminateGL);
            ...
            propertyNotification.Notified += (object source, PropertyNotification.NotifyEventArgs args) =>
            {
                ...
                if (target != null)
                {
                    Interop.ExtRenderer.UpdateGLWindowSize((int)target.SizeWidth, (int)target.SizeHeight);
