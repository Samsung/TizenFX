...
private static object MinReachedEvtKey = new object();
    ...
    public event EventHandler MinReachedEvt
    {
        add {
            lock (eventLock) {
                ...
                if (AddNativeEventHandler(efl.Libs.Elementary, key, this.evt_MinReachedEvt_delegate)) {
        ...
        remove {
            lock (eventLock) {
                string key = "_EFL_UI_SPIN_EVENT_MIN_REACHED";
                if (RemoveNativeEventHandler(key, this.evt_MinReachedEvt_delegate)) { 
    ...
    private void on_MinReachedEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
        EventArgs args = EventArgs.Empty;
        try {
...
private static object MaxReachedEvtKey = new object();
    ...
    public event EventHandler MaxReachedEvt
    {
        add {
            lock (eventLock) {
                ...
                if (AddNativeEventHandler(efl.Libs.Elementary, key, this.evt_MaxReachedEvt_delegate)) {
        ...
        remove {
            lock (eventLock) {
                string key = "_EFL_UI_SPIN_EVENT_MAX_REACHED";
                if (RemoveNativeEventHandler(key, this.evt_MaxReachedEvt_delegate)) { 
    ...
    private void on_MaxReachedEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
        EventArgs args = EventArgs.Empty;
        try {
