...
/// <summary>Special <see cref="Efl.Ui.Item"/> that acts as group header.
/// Items in this group have to be added directly to the <see cref="Efl.Ui.GroupItem"/> using the <see cref="Efl.IPackLinear"/> interface, instead of being added to the container. For example, an <see cref="Efl.Ui.List"/> container where all items are inside a group will contain only <see cref="Efl.Ui.GroupItem"/> objects. Normal <see cref="Efl.Ui.Item"/> objects will be contained inside the group items.
/// 
/// Each container chooses how group headers are rendered. See for example <see cref="Efl.Ui.List"/> or <see cref="Efl.Ui.Grid"/>.</summary>
