...
private static object AnimateBeginEvtKey = new object();
    ...
    public event EventHandler AnimateBeginEvt
    {
        add {
            lock (eventLock) {
                ...
                if (AddNativeEventHandler(efl.Libs.Elementary, key, this.evt_AnimateBeginEvt_delegate)) {
        ...
        remove {
            lock (eventLock) {
                string key = "_EFL_UI_FLIP_EVENT_ANIMATE_BEGIN";
                if (RemoveNativeEventHandler(key, this.evt_AnimateBeginEvt_delegate)) { 
    ...
    private void on_AnimateBeginEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
...
private static object AnimateDoneEvtKey = new object();
    ...
    public event EventHandler AnimateDoneEvt
    {
        add {
            lock (eventLock) {
                ...
                if (AddNativeEventHandler(efl.Libs.Elementary, key, this.evt_AnimateDoneEvt_delegate)) {
        ...
        remove {
            lock (eventLock) {
                string key = "_EFL_UI_FLIP_EVENT_ANIMATE_DONE";
                if (RemoveNativeEventHandler(key, this.evt_AnimateDoneEvt_delegate)) { 
    ...
    private void on_AnimateDoneEvt_NativeCallback(System.IntPtr data, ref Efl.Event.NativeStruct evt)
    {
        EventArgs args = EventArgs.Empty;
        try {
