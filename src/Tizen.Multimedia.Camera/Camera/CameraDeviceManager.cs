    ...
    public class CameraDeviceManager : IDisposable
    {
        ...
        public IEnumerable<CameraDeviceInformation> SupportedDevices
        {
            get
            {
                ...
                Native.SupportedDeviceCallback callback = (type, device, name, id, streamNum, _) =>
                {
                    try
                    {
                        Log.Info(CameraLog.Tag,
                            $"Type:{type.ToString()}, Device:{device.ToString()}, Name:{name}, Id:{id}, NumberOfExtraStream:{streamNum}");

                        deviceList.Add(new CameraDeviceInformation(type, device, name, id, streamNum));
                        return true;
                    }
                    catch (Exception e)
                    {
        ...
        {
            _deviceConnectionChangedCallback = (type, device, name, id, streamNum, isConnected, _) =>
            {
                Log.Debug(CameraLog.Tag, "Invoke DeviceConnectionChanged event");
                var deviceInfo = new CameraDeviceInformation(type, device, name, id, streamNum);
