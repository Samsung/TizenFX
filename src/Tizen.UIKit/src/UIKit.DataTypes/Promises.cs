...
using static UIKit.DataTypes.EinaNative.PromiseNativeMethods;
...
static internal class PromiseNativeMethods
{
    ...
    [DllImport(UIKit.Libs.Eina)]
    internal static extern void eina_promise_resolve(IntPtr scheduler, UIKit.DataTypes.ValueNative value);

    [DllImport(UIKit.Libs.Eina)]
    internal static extern void eina_promise_reject(IntPtr scheduler, UIKit.DataTypes.Error reason);

    [DllImport(UIKit.Libs.CustomExports)]
    internal static extern void efl_mono_thread_safe_promise_reject(IntPtr scheduler, UIKit.DataTypes.Error reason);
    ...
    internal delegate UIKit.DataTypes.ValueNative FutureCb(IntPtr data, UIKit.DataTypes.ValueNative value, IntPtr dead_future);
...
public class Promise : IDisposable
{
    ...
    {
        UIKit.Loop loop = UIKit.App.AppMain;
    ...
    private static void NativeCancelCb(IntPtr data, IntPtr dead)
    {
        ...
        {
            UIKit.DataTypes.Log.Info("Null promise CancelCb found");
        }
    ...
    /// <summary>Disposes of this wrapper, rejecting the native promise with <see cref="UIKit.DataTypes.Error.ECANCELED"/></summary>
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (Handle != IntPtr.Zero)
        {
            if (disposing)
            {
                eina_promise_reject(Handle, UIKit.DataTypes.Error.ECANCELED);
            }
            else
            {
                efl_mono_thread_safe_promise_reject(Handle, UIKit.DataTypes.Error.ECANCELED);
            }
            Handle = IntPtr.Zero;
        }
    }
    ...
    /// </summary>
    public void Resolve(UIKit.DataTypes.Value value)
    {
        SanityChecks();
    ...
    /// Rejects a promise.
    ///
    /// The future chain attached to this promise will be called with an UIKit.DataTypes.Value of type
    /// UIKit.DataTypes.ValueType.Error and payload UIKit.DataTypes.Error.ECANCELED.
    /// </summary>
    public void Reject(UIKit.DataTypes.Error reason)
    {
        SanityChecks();
...
public class Future
{
    ...
    ///
    /// The UIKit.DataTypes.Value as argument can come with an UIKit.DataTypes.Error.ECANCELED as payload if the
    ...
    /// </summary>
    public delegate UIKit.DataTypes.Value ResolvedCb(UIKit.DataTypes.Value value);
    ...
    {
        handle = ThenRaw(handle, (UIKit.DataTypes.Value value) =>
        {
            Handle = IntPtr.Zero;
            return value;
        });
    ...
    public Future(Promise promise, ResolvedCb cb = null)
    {
        ...
        Handle = ThenRaw(intermediate, (UIKit.DataTypes.Value value) =>
        {
            if (cb != null)
            {
                value = cb(value);
    ...
    ///
    /// The callbacks will still be called with UIKit.DataTypes.Error.ECANCELED as payload. The promise cancellation
    ...
    private static UIKit.DataTypes.ValueNative NativeResolvedCb(IntPtr data, UIKit.DataTypes.ValueNative value, IntPtr dead_future)
    {
        GCHandle handle = GCHandle.FromIntPtr(data);
        ResolvedCb cb = handle.Target as ResolvedCb;
        if (cb != null)
        {
            UIKit.DataTypes.Value managedValue = cb(value);
        ...
        {
            UIKit.DataTypes.Log.Warning("Failed to get future callback.");
        }
    ...
    public Future Chain(IEnumerable<ResolvedCb> cbs)
    {
        ...
        catch (Exception e)
        {
            ...
            UIKit.DataTypes.Log.Error($"Failed to create native future description for callbacks. Error: {e.ToString()}");
            return null;
        }
...
/// <summary>Custom marshaler to convert between managed and native <see cref="UIKit.DataTypes.Future"/>.
...
public class FutureMarshaler : ICustomMarshaler
{
    ...
    ///<returns>An <see cref="UIKit.DataTypes.Future"/> wrapping the native future.</returns>
    ...
    ///<param name="managedObj">The managed wrapper. If it is not an <see cref="UIKit.DataTypes.Future"/>, the value returned
    ...
    ///<returns>The <see cref="UIKit.DataTypes.FutureMarshaler"/> instance that will marshall the data.</returns>
