...
namespace Tizen.NUI.ViewAnnotationSample
{
    ...
    class Program : NUIApplication
    {
        ...
        private void Annotate(View view, string entityId, string name, string role)
        {
            view.SetAnnotation(entityId, EntityType, MakeAppEntityInfo(entityId, name, role));
        }

        private void ShowAnnotation(View view, string name)
        {
            if (view.GetAnnotation(out string entityId, out string entityType, out string entityInfo))
            {
                statusLabel.Text = string.IsNullOrEmpty(entityInfo)
                    ? string.Format("{0}: {1} ({2})", name, entityId, entityType)
                    : string.Format("{0}: {1} ({2}) [{3}]", name, entityId, entityType, entityInfo);
            }
            else
            {
