...
using static UIKit.DataTypes.IteratorNativeFunctions;
...
public class Hash<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>, IDisposable
{
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        if (Own)
        {
            ...
            {
                UIKit.Wrapper.Globals.ThreadSafeFreeCbExec(eina_hash_free, h);
            }
        }
    }
    ...
    private static bool ForceRefKey<T>()
    {
        return (!typeof(T).IsValueType)
               ...
               && (typeof(T) != typeof(UIKit.DataTypes.Stringshare));
    }

    private static IntPtr CopyNativeObject<T>(T value, bool forceRef)
    {
        if (!IsUIKitObject(typeof(T)) && forceRef)
        {
        ...
        else if (IsUIKitObject(typeof(T)) && forceRef)
        {
    ...
    public UIKit.DataTypes.Iterator<TKey> Keys()
    {
        return new UIKit.DataTypes.Iterator<TKey>(EinaHashIteratorKeyNew<TKey>(Handle), true);
    }

    public UIKit.DataTypes.Iterator<TValue> Values()
    {
        return new UIKit.DataTypes.Iterator<TValue>(eina_hash_iterator_data_new(Handle), true);
    }
    ...
    public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
    {
        ...
        try
        {
            ...
            {
                var tuple = Marshal.PtrToStructure<UIKit.DataTypes.HashTupleNative>(tuplePtr);
