...
public partial class FunctionInterop
{
    ...
    public static IntPtr LoadFunctionPointer(string moduleName, string functionName)
    {
        ...
        UIKit.DataTypes.Log.Debug($"searching {module} for {functionName}");
        var s = FunctionInterop.dlsym(module, functionName);
        UIKit.DataTypes.Log.Debug($"searching {module} for{functionName}, result {s}");
        return s;
    }
    ...
    {
        UIKit.DataTypes.Log.Debug($"searching {null} for {functionName}");
        var s = FunctionInterop.dlsym(IntPtr.Zero, functionName);
        UIKit.DataTypes.Log.Debug($"searching {null} for {functionName}, result {s}");
        return s;
    }
}
