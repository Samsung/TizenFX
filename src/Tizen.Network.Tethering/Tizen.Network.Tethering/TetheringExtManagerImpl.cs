...
namespace Tizen.Network.Tethering
{
    ...
    internal partial class TetheringExtManagerImpl
    {
        ...
        internal SafeTetheringExtManagerHandle Initialize()
        {
            ...
            {
                Log.Error(Globals.LogTag, "Initialize Fail, Error - " + (TetheringExtError)ret);
                TetheringExtErrorFactory.ThrowTetheringExtException(ret, PrivilegeNetworkGet);
            }
            return handle;
        }
        ...
        private void CheckReturnValue(int ret, string method, string privilege)
        {
            if (ret != (int)TetheringExtError.None)
            {
                Log.Error(Globals.LogTag, method + " Fail, Error - " + (TetheringExtError)ret);
                TetheringExtErrorFactory.ThrowTetheringExtException(ret, privilege);
            }
        }
    }
}
