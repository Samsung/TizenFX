...
using static UIKit.DataTypes.HashNativeFunctions;
using static UIKit.DataTypes.InarrayNativeFunctions;
using static UIKit.DataTypes.InlistNativeFunctions;
...
public class StringElementTraits : IBaseElementTraits<string>
{
    ...
    public string NativeToManagedInlistNode(IntPtr nat)
    {
        if (nat == IntPtr.Zero)
        {
            UIKit.DataTypes.Log.Error("Null pointer for Inlist node.");
            return default(string);
        }
...
public class StringshareElementTraits : IBaseElementTraits<UIKit.DataTypes.Stringshare>
{
    public StringshareElementTraits()
    {
    }

    public IntPtr ManagedToNativeAlloc(UIKit.DataTypes.Stringshare man)
    {
        var strShare = MemoryNative.AddStringshare(man);
        return strShare;
    }

    public IntPtr ManagedToNativeAllocInlistNode(UIKit.DataTypes.Stringshare man)
    {
        var node = new InlistNode<IntPtr>();
        node.Val = ManagedToNativeAlloc(man);
        GCHandle pinnedData = GCHandle.Alloc(node, GCHandleType.Pinned);
    ...
    public void ManagedToNativeCopyTo(UIKit.DataTypes.Stringshare man, IntPtr mem)
    {
        IntPtr stringptr = ManagedToNativeAlloc(man);
        Marshal.WriteIntPtr(mem, stringptr);
    }
    ...
    public UIKit.DataTypes.Stringshare NativeToManaged(IntPtr nat)
    {
        if (nat == IntPtr.Zero)
        {
            return default(UIKit.DataTypes.Stringshare);
        }

        return StringConversion.NativeUtf8ToManagedString(nat);
    }

    public UIKit.DataTypes.Stringshare NativeToManagedInlistNode(IntPtr nat)
    {
        if (nat == IntPtr.Zero)
        {
            UIKit.DataTypes.Log.Error("Null pointer for Inlist node.");
    ...
    // Strings inplaced are always a pointer, because they are variable-sized
    public UIKit.DataTypes.Stringshare NativeToManagedInplace(IntPtr nat)
    {
        if (nat == IntPtr.Zero)
        {
            return default(UIKit.DataTypes.Stringshare);
...
public class UIKitObjectElementTraits<T> : IBaseElementTraits<T>
{
    public IntPtr ManagedToNativeAlloc(T man)
    {
        IntPtr h = ((UIKit.Wrapper.IWrapper)man).NativeHandle;
        if (h == IntPtr.Zero)
        {
            return h;
        }

        return UIKit.Wrapper.Globals.efl_ref(h);
    }
    ...
    public void NativeFree(IntPtr nat)
    {
        if (nat != IntPtr.Zero)
        {
            UIKit.Wrapper.Globals.efl_mono_thread_safe_efl_unref(nat);
        }
    }
    ...
    public T NativeToManaged(IntPtr nat)
    {
        ...
        return (T) UIKit.Wrapper.Globals.CreateWrapperFor(nat, shouldIncRef: true);
    }
    ...
    public IntPtr EinaFreeCb()
    {
        return MemoryNative.UIKitUnrefFuncPtrGet();
    }
...
{
    private UIKit.DataTypes.Callbacks.EinaCompareCb dlgt = null;
    ...
    public T NativeToManaged(IntPtr nat)
    {
        if (nat == IntPtr.Zero)
        {
            UIKit.DataTypes.Log.Error("Null pointer on primitive/struct container.");
            return default(T);
        }
    ...
    public IntPtr EinaCompareCb()
    {
        if (dlgt == null)
        {
            dlgt = new UIKit.DataTypes.Callbacks.EinaCompareCb(PrimitiveCompareCb);
        }
...
{
    public static bool IsUIKitObject(System.Type type)
    {
        return typeof(UIKit.Wrapper.IWrapper).IsAssignableFrom(type);
    }
    ...
    {
        return type == typeof(UIKit.DataTypes.Stringshare);
    }

    public static UIKit.DataTypes.ElementType GetElementTypeCode(System.Type type)
    {
        if (IsUIKitObject(type))
        {
    ...
    private static System.Type AsUIKitInstantiableType(System.Type type)
    {
        if (!IsUIKitObject(type))
        {
            return null;
    ...
    {
        UIKit.DataTypes.Log.Debug($"Finding TypeTraits for {typeof(T).Name}");
        object traits;
        var type = typeof(T);
        if (IsUIKitObject(type))
        {
            System.Type concrete = AsUIKitInstantiableType(type);
            ...
            traits = new UIKitObjectElementTraits<T>();
        }
        else if (IsString(type))
        {
