...
namespace Tizen.NUI.WindowSystem
{
    ...
    public class EdgeDragData : IDisposable
    {
        ...
        /// <summary>
        /// Creates a new EdgeDragData.
        /// </summary>
        /// <exception cref="ArgumentException">Thrown when failed of invalid argument.</exception>
        /// <exception cref="ArgumentNullException">Thrown when a argument is null.</exception>
        public EdgeDragData(InputGesture gesture, int fingers, GestureEdge edge)
        {
            if (gesture == null)
            {
            ...
            _gesture = gesture;
            _handle = EdgeDragNew(_gesture.GetNativeHandle(), fingers, (int)edge);
            if (_handle.IsInvalid)
            {
                int err = Tizen.Internals.Errors.ErrorFacts.GetLastResult();
                _gesture.ErrorCodeThrow((Interop.InputGesture.ErrorCode)err);
            }
            Log.Debug(LogTag, "EdgeDragData Created");
        }
        ...
        {
            Interop.InputGesture.ErrorCode res = Interop.InputGesture.EdgeDragSizeSet(_handle, (int)edgeSize, startPoint, endPoint);
            _gesture.ErrorCodeThrow(res);
            Log.Debug(LogTag, "EdgeDragData.SetEdgeSize");
        }
        ...
        {
            Dispose(false);
            Log.Debug(LogTag, "EdgeDragData Destroyed");
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposed)
            {
                Interop.InputGesture.ErrorCode res = Interop.InputGesture.EdgeDragFree(_gesture.GetNativeHandle(), _handle);
                _gesture.ErrorCodeThrow(res);
